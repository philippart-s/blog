...
Bien entendu cela permet d'éviter des actions fastidieuses de configuration pour :
 ...
 - partager une confiuration pour un projet open source 
...
Juste se connecter sur https://gitpod.io et créer un compte ou utiliser un de vos compte GitHub, GitLab, Bitbucket.

## ConfigurationS
Gitpod est donc un IDE basé sur Eclipse Teia dans une image.
L'image par [défaut](https://github.com/gitpod-io/workspace-images/blob/master/full/Dockerfile){:target="_blank"} est très complète et permet déjà de faire pas mal de chose : python, java, node, go, ...
...
Gitpod vient avec la notion de workspaces, qui représente en gros une instance de Gitpod sur une branche donnée.
...
On peut avoir besoin de configurer de manière spécifique un projet et donc ses workspaces associés, pour cela tout va se passer dans le fichier `.gitpod.yml` à placer à la racine du projet.
...
Voici un exemple de d'initialisation que l'on peut faire, dans mon cas je récupère les différentes dépendances pour Jekyll :
```yml
tasks:
  - name: Init
    init: bundle install
...
Avec ces trois exemples on voit que l'on a deux types de paramétrages : 
 - ceux qui sont propre au projet / workspace, ici le fait d'utiliser Ruby,
 - ceux communs à tous les projets / workspace, ici la configuration wakatime et les extensions.
...
```
 - créer une image, la publier sur une registry et la référencée dans le `.gitpod.yml` : 
```yml
...
Personnellement j'utilise la synchronisation via VSCode mais celle fournie par Gitpod est intéressante car elle permet de choisir si c'est partagé par tous (propre au projet), pour tous les workspaces de l'utilisateur ou pour un workspace en particulier.
...
```
Dans cet exemple le projet installera automatiquement l'extension _vscode-maven_ par exemple dans le cas où on voudrait être sûr de l'avoir si on est sur un projet Java.
Pour savoir quel est le nom de l'extension à mettre dans le fichier `.gitpod.yml` c'est assez simple car VSCode nous le propose directement sur l'extension (roue crantée puis _Add to .gitpod.yml_).
...
N'hésitez pas à aller jeter un oeuil et si vous voyez des coquilles n'hésitez pas !
...
Je vous conseille aussi les ressources suivantes : 
 ...
 - de jeter un oeuil sur le [compte GitLab](https://gitlab.com/k33g_org/k33g_org.gitlab.io/-/issues){:target="_blank"} de Philippe Charrière
...
J'espère vous avoir donné envi de jeter un oeuil à ce type d'IDE.
J'atais le premier septique mais j'avoue que j'ai vraiment été bluffé par la facilité d'utilisation et de customisation.
Je n'en suis pas à mettre à la pouvbelle tous mes IDE mais j'avoue que pour plein de situations je passe de plus en plus par Gitpod !
