...
Tout ça pour dire que, vous le constaterez, le dialogue homme-machine n'est pas tout jeune, voir il remonte aux années 60-70 où on avait besoin d’interagir avec les mainframes (pour les plus jeunes : l'équivalent d'une maison remplie de serveurs pour avoir à peine la puissant de votre téléphone 😅).
...
Notez ici, que ce mode a quelques inconvénients : 
 ...
 - ⬇ tout ceci est en mode _pull_ : le model renvoie de la donnée en "lecture seule"
...
Cela fonctionne bien pour spécialiser un modèle sur des données qu'il n’aurait pas vu, mais c'est assez mauvais avec des données en faible quantité qui traiteraient des informations en temps réel.
...
> Si vous voulez voir comment implémenter du RAG, voici quelques liens d'articles que j'ai écris dans le cadre de mon activité professionnelle 😉 :
...
Et quand je dis n'importe quelle, je ne parle pas que des données publiques mais aussi celles de votre entreprise ou de vous données personnelles.
...
Le function calling va permettre de faire cet ajout de données mais il va surtout permettre d'appeler n'importe quelle outil et de faire n'importe quelle action : Create, Read, Update et Delete.

On y vient à mon interface Homme - Machine.

> **⚠️ Dès à présent pour la suite de l'article retenez bien une chose : ce n'est JAMAIS et non JAMAIS le modèle qui appel les outils mais bien l'application appelante (celle qui fait la requête au modèle) qui le fait. ⚠️**
...
Suite à cela le modèle va ou non indiquer quels outils et paramètres qu'il faut utiliser, en fonction de la teneur du prompt, à l'application appelante et c'est elle qui va déclencher les appels.
...
Je déclare donc une fonction `enregistrerPersonne` qui prends en paramètre des chaines de caractères représentant le nom, le prénom et l'email de la personne.
...
Il reste toujours des problèmes, bien sûr : 
 - l'appel de la fonction n'est pas déterministe : même si vous pouvez jouer sur des paramètres lors de l'appel à votre modèle (comme la température ou le paramètre top_P par exemple), vous ne serez jamais sûr à 100% qu'il fasse les bons choix ... souvenez les statistiques et tout ça 😉,
...
> Si vous voulez voir comment implémenter du function calling, voici un lien vers un article que j'ai écris dans le cadre de mon activité professionnelle 😉 : [Using Function Calling with OVHcloud AI Endpoints](https://blog.ovhcloud.com/using-function-calling-with-ovhcloud-ai-endpoints/){:target="_blank"}
...
C'est un très gros résumé de ce que propose MCP : la création d'un protocol vous permettant de récupérer, entre autre, la liste des outils d'un ou de plusieurs fournisseurs.
...
Car oui ... on s'extase devant une architecture 3 tiers qui a plus de 20 ans ... Interface, logique métier, données.

Mais attention, loin de moi de dire que ce n'est pas une vrai avancée, le MCP et toutes ses variantes concurrentes apportent, pour moi, la dernière pierre pour que l'on repense totalement la façon dont l'Homme interagi avec la machine.
...
> Si vous voulez voir comment implémenter un serveur MCP, voici un lien vers un article que j'ai écris dans le cadre de mon activité professionnelle 😉 : [Model Context Protocol (MCP) with OVHcloud AI Endpoints](https://blog.ovhcloud.com/model-context-protocol-mcp-with-ovhcloud-ai-endpoints/){:target="_blank"}
...
C'est juste, qu'après ces quelques années passées dans l'IA et plus particulièrement dans la gen AI avec les LLM je me rends compte que l'on se rapproche de plus en plus vers une nouvelle façon d'échanger avec nos machines.
...
A ce sujet, je pense que le function calling avec ou sans MCP commence à être une réponse pour diminuer la taille des modèles, en les améliorant, à la demande, avec des outils (tient c'est marrant c'est qu'est sensée faire l'IA avec l'humain non ?).
